...
STEPS :
    ...
    9) Now,  crop PAM50 to have a template that is not the whole spine otherwise too long
    (this is in the local directory on the Lumbar project on the server ~/PhDProjects/Lumbar/templates)
    
    run sct_warp_template -d PAM50_t2_cropped.nii.gz -w transform_identity.txt -s 1 -ofolder PATH_OUTPUT

    10) run a_normalize_after_TA.sh and b_normallize_after_TA.m
    11) run Clustering on all subjects list
    12) run post_iCAPs for Native so that you have 
...
# ADD QUALITY CONTROL IN FUNCTIONS  
...
class PreprocessingRS(object):
    ...
    def _concat_transfo(self, sps):
        ...
        run_string = 'cd %s; cd Normalization; \
                      sct_concat_transfo -w warp_fmri2anat.nii.gz ../../anat/warp_anat2template.nii.gz -o warp_fmri2template.nii.gz -d %s/data/PAM50/template/PAM50_t2.nii.gz;\
                      sct_concat_transfo -w ../../%s/warp_template2anat.nii.gz warp_anat2fmri.nii.gz -o warp_template2fmri.nii.gz -d ../mfmri_mean.nii.gz' % (sps, self.SCT_PATH, self.anat)
    ...
    # def __static_plot(self, card_signal, seq_card):

    #     ys = [j for j in seq_card]
    #     fig = plt.figure()
    #     a = fig.add_subplot(111)
    #     a.plot(card_signal)
    #     a.vlines(ys, -0.1, 0.1, colors='r')
    ...
    def _fsl_motion_outliers(self, sps):
        ...
        run_string = 'cd %s; fsl_motion_outliers -i mfmri.nii.gz -o outliers.txt —m ../Segmentation/mask_sc.nii.gz -p outliers.png --dvars --nomoco;' % (sps)
        print(run_string)
        os.system(run_string)
        ...
        os.system('export DIREC=%s; bash %s/noisereg_subtemplate.sh' % (sps, self.fsl_template_dir))
