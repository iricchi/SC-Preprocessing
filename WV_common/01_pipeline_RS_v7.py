...
class PreprocessingRS(object):
    ...
    def __init_configs(self):
        ...
        self.card_demedian = False  # by default don't remove median from cardiac data
        self.mode = ''
        self.fsessions_name = ''
    ...
    def _convert_txt_filt(self, sub):
        ...
        if self.filter_cardiac:
            ...
            if Wn > 1.0:
                Wn = 0.99
            B,A = butter(3,Wn,'low')
            cardsig = data[:,int(self.pnm_columns['cardiac'])-1]
            if self.card_demedian:
                cardsig = cardsig - np.median(cardsig)

            data[:,int(self.pnm_columns['cardiac'])-1] = filtfilt(B,A,cardsig)
    ...
    def _normalization(self, sps, fmriname="fmri"):
        ...
        run_string = f'cd {sps}; sct_apply_transfo -i {denoised_name}.nii.gz -d {pam50_template_full} -w Normalization/warp_{self.fmriname}2template{add}.nii.gz\
